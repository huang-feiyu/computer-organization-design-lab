...
int verify(double *x, double *y,
    ...
    (*funct)(x, y, z_v_add);
    for (int i = 0; i < ARRAY_SIZE; i++) z_oracle[i] = x[i] + y[i];
...
int main() {
    ...
    //     for (int j = 0; j < REPEAT; j++) v_add_optimized_adjacent(x, y, z);
    ...
    for (int i = 1; i <= num_threads; i++) {
        omp_set_num_threads(i);
        start_time = omp_get_wtime();
        for (int j = 0; j < REPEAT; j++) v_add_optimized_chunks(x, y, z);
        run_time = omp_get_wtime() - start_time;
    ...
    for (int i = 1; i <= num_threads; i++) {
        omp_set_num_threads(i);
        start_time = omp_get_wtime();
        for (int j = 0; j < REPEAT; j++) v_add_naive(x, y, z);
        run_time = omp_get_wtime() - start_time;
